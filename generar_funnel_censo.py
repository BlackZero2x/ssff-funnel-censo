...
MAEST_PATH     = 'C:/proyectos/SSFF/maestro_clientes_SSFF.xlsx'   # fallback si falla SQL
TABLAS_PATH    = 'C:/proyectos/SSFF/TABLAS_RUTAS.xlsx'
OUTPUT         = 'C:/proyectos/SSFF/funnel_censo_SSFF.xlsx'
ENCUESTAS_PATH = 'C:/proyectos/SSFF/encuestas_mayo2026.csv'   # fallback si falla MySQL
...
# Antes de las 10am usar ayer como fecha de referencia (evita proyectado parcial en envío 6am)
...
funnel_rows = [
    ('UNIVERSO — Total clientes censo',
     n_censo_total, 1.0, None, C_BLANCO, True),
    (f'COBERTURADOS — Con compra Mar{"-May" if tiene_mayo else "-Abr"} 2026',
     cob_total, _pct(cob_total,n_censo_total), sol_total, C_AZUL_CLA, True),
    ('COBERTURADOS MAR-26 — Marzo 2026',
     cob_mar, _pct(cob_mar,n_censo_total), sol_mar, C_BLANCO, False),
    ('COBERTURADOS ABR-26 — Abril 2026',
     cob_abr, _pct(cob_abr,n_censo_total), sol_abr, C_BLANCO, False),
]
if tiene_mayo:
    funnel_rows.append((
        'COBERTURADOS MAY-26 — Mayo 2026 (en curso)',
        cob_may, _pct(cob_may,n_censo_total), sol_may, C_AMARILLO, False))
else:
    funnel_rows.append((
        'COBERTURADOS MAY-26 — Sin datos',
        None, None, None, C_AMARILLO, False))
funnel_rows.append((
    'SIN COBERTURA — Sin compra post-reestructuracion',
    sin_cob, _pct(sin_cob,n_censo_total), 0, C_ROJO_CLA, True))
